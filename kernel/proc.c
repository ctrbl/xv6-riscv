...
int nextpid = 1;
int scheduler_type = 0;   // default scheduling
...
// Extend wait system calls to print out creation time, end time, and total time of process in test.c
int
waitx(uint64 addr, int* wait_time, int* turnaround_time)
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          // Copy process time to user space 
